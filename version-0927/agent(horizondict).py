...
for i in range(1, 11):
    if i % 2 == 1:
        horizonValue[(i, (i + 1) / 2)] = 0
        for j in range(1, i + 1):
            horizonValue[(i, j)] = abs(0.75 * (j - (i + 1) / 2))

    else:
        horizonValue[(i, i / 2)] = 0
        horizonValue[(i, i / 2 + 1)] = 0
        for j in range(1, i + 1):
            horizonValue[(i, j)] = ((abs(j - (i + 1) / 2)- 0.5)) * 0.75

for i in range(11, 20):
    if i % 2 == 1:
        k = 20 - i
        horizonValue[(i, (k + 1) / 2)] = 0
        for j in range(1, k + 1):
            horizonValue[(i, j)] = abs(0.75 * (j - (k + 1) / 2))

    else:
        k = 20 - i
        horizonValue[(i, k / 2)] = 0
        horizonValue[(i, k / 2 + 1)] = 0
        for j in range(1, k + 1):
            horizonValue[(i, j)] = (abs(j - (k + 1) / 2) - 0.5) * 0.75
...
def eval(state, player):  # this function should return a number as the evaluation value of a given state
    ...
    for position in player_status:
        player_horizontal_count += horizonValue[position]#abs(abs(position[1] - (11 - abs(position[0] - 10)) / 2) - 1)  # 1 -> 5.5 -> 1

    opponent_horizontal_count = 0
    for position in opponent_status:
        opponent_horizontal_count += horizonValue[position]#abs(abs(position[1] - (11 - abs(position[0] - 10)) / 2) - 1)
    ...
    if player == 1:
        ...
        if opponent_vertical_count == 140:  # you lose!
            ...
            return 280 - (player_vertical_count + opponent_vertical_count) + (
        ...
        if opponent_vertical_count == 0:
            ...
            return (player_vertical_count + opponent_vertical_count) + (
                        opponent_horizontal_count - player_horizontal_count) / 2
