...
def runGame(ccgame, agents):
    state = ccgame.startState() # ChineseChecker(10, 4)
    print(state)
    ...
    while (not ccgame.isEnd(state, iter)) and iter < max_iter:
        iter += 1
        # board 是 GameBoard 类的
        # state = ( 1, self.board)，self.board 是checker游戏的一个属性
        board.board = state[1]   
        board.draw()
        board.update_idletasks() 
        board.update()

        player = ccgame.player(state)  # player = state[0]
        # agents = agents_dict {1: simpleGreedyAgent1, 2: simpleGreedyAgent}
        agent = agents[player]
        # function agent.getAction() modify class member action
        # 1 s return

        timeout(agent.getAction, state)
        # 尝试调用 getAction(state)， 
...
def callback(ccgame):
    B.destroy()
    TeamNameMinimaxAgent1 = TeamNameMinimaxAgent(ccgame)
    MarbleFish2 = MarbleFish(ccgame)
    TeamNameMinimaxAgent2 = TeamNameMinimaxAgent(ccgame)
    MarbleFish1 = MarbleFish(ccgame)
    SimpleGreedyAgent1 = SimpleGreedyAgent(ccgame)
    SimpleGreedyAgent2 = SimpleGreedyAgent(ccgame)
    simulateMultipleGames({1: MarbleFish1, 2: TeamNameMinimaxAgent2}, 1, ccgame)
